...
function title() {
    console.log(chalk.bold.green(figlet.textSync('Xlycans Bot', {
        font: 'Standard',
...
function uncache(module = '.') {
    ...
    const haruka = harukaConnect({
      version,
        ...
        browser: ['Xlycans Multi Device','Safari','1.0.0'],
        auth: state
    })
...
app.use(express.static(path.join(__dirname, 'views')))
  server.listen(PORT, () => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            ...
            else if (reason === DisconnectReason.timedOut) { console.log("xlycansection TimedOut, Reconnecting..."); startHaruka(); }
